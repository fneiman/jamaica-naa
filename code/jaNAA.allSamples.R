...
# jaNAA.allSamples.R
# R code for the analyis of NAA samples from Jamaica
# includes all samples -- no screening for outliers
...
# 2. Identify and impute missing values #### 
...
par(mfrow=c(1,1))
#plot PC1 and 2
plot(getScores(pca)[,1],getScores(pca)[,2],
     pch = ja$coastPch, 
     bg = adjustcolor(ja$estateCol, alpha=.5), 
     col="black", 
     cex=2, cex.lab= 1.5, xlab="PC 1", ylab="PC 2")

text(getScores(pca)[labelIndex,1],
     getScores(pca)[labelIndex,2],
     logJa$ANID[labelIndex], cex=.5)


#plot the variables on PC1 and PC2
plot(getLoadings(pca)[,1],getLoadings(pca)[,2],  type="n",
     cex=2, cex.lab= 1.5, xlab="PC 1", ylab="PC 2",
     xlim= c(-.4, .4), ylim= c(-.4, .4))
# Plot arrows: see ?arrows for the syntax 
arrows(0, 0, getLoadings(pca)[,1],getLoadings(pca)[,2], len=.2, lwd=2, col="grey")
abline(h=0,v=0,col="black", lwd=1, lty=2)
text(1.05* getLoadings(pca)[,1] , 1.05* getLoadings(pca)[,2] , names(getLoadings(pca)[,1]),
     col="black", cex=1.5)


#plot PC 1 and 3
plot(getScores(pca)[,1],getScores(pca)[,3],
     pch = ja$coastPch, 
     bg = adjustcolor(ja$estateCol, alpha=.5),
     col="black", 
...
text(getScores(pca)[labelIndex,1],
     getScores(pca)[labelIndex,3],
     logJa$ANID[labelIndex], cex=.5)



#plot the variables on PC1 and PC3
plot(getLoadings(pca)[,1],getLoadings(pca)[,3],  type="n",
     cex=2, cex.lab= 1.5, xlab="PC 1", ylab="PC 3")

# Plot arrows: see ?arrows for the syntax 
arrows(0, 0, getLoadings(pca)[,1],getLoadings(pca)[,3], len=.2, lwd=2, col="grey")
abline(h=0,v=0,col="black", lwd=1, lty=2)
pointLabel(1.05* getLoadings(pca)[,1] , 1.05* getLoadings(pca)[,3] , names(getLoadings(pca)[,1]),
     col="black", cex=1)
...
plot3d(getScores(pca)[,1],
       ...
       zlab ="PC 3",
       cex.lab=1.5,
       adjustcolor(ja$estateCol, alpha=.5),
       type="s",
       size=1)

text3d(getScores(pca)[,1],
       getScores(pca)[,2],
       getScores(pca)[,3],
       logJa$ANID,
       adj = c(-.5,.5))
...
pc1<-prcomp(~ As + La + Lu + Nd + Sm + U +  Yb + Ce + Co + Cr + Cs + Eu + Fe + Hf + Rb + Sb + 
              Sc + Sr + Ta + Tb + Th + Zn + Zr + Al + Ba + Ca + Dy + K +  Mn + Na + Ti + V , 
            data=logJa, retx = TRUE, center = TRUE, scale. = T)
...
silPlot <- silAvgWidth(data = logJa[,-1] , maxK=20)
...
#  k=7 or k= 8. so we go with 8


library(cluster)
pam.K <- pam(logJa[,-1], k=8)
...
LDAfit.cv <- lda(pam.K$clustering ~ As + La + Lu + Nd + Sm + U +  Yb + Ce + Co + Cr + Cs + Eu + Fe + Hf + Rb + Sb + 
                Sc + Sr + Ta + Tb + Th + Zn + Zr + Al + Ba + Ca + Dy + K +  Mn + Na + Ti + V, 
              data=logJa, 
              na.action="na.omit",  CV=T, method="moment", 
              prior= rep(1,max(pam.K$clustering))/max(pam.K$clustering) )
